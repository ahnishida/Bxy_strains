...
tree_file <- 'results/pangenome/Bacteroides_xylanisolvens_outgroup/phylogeny/RAxML_bipartitions.Bacteroides_xylanisolvens_outgroup.rooted'
table_file <- 'results/pangenome/Bacteroides_xylanisolvens_outgroup/roary/gene_presence_absence.csv'
outdir <- 'results/pangenome/Bacteroides_xylanisolvens_outgroup/count'
system(paste0('rm -r ',outdir))
system(paste0('mkdir -pv ',outdir))
...
table_no_orfans <- table %>% 
  mutate(sum = rowSums(across(where(is.numeric)))) %>% 
  filter(sum>1) %>% 
  select(-sum)
write.table(table_no_orfans,file=file.path(outdir,'pres_abs_no_orfans.txt'),
            sep='\t',quote=FALSE,row.names = FALSE)
...
data <- file.path(outdir,'pres_abs_no_orfans.txt')
output <- file.path(outdir,'Bxy_count_no_orfans')
...
output_prefix <- file.path(outdir,'Bxy_count')
runCount(tree,data,output_prefix,1.5)

#visualize count output
wag <- read.table(file.path(outdir,'Bxy_count_no_orfans.CHANGE'),sep='\t',header=TRUE)
