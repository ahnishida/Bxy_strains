...
Outputs summary files to be used by figures_tables.Rmd
...
identify_pw_50strain = function(species) {
  ...
  tree_distM<- cophenetic.phylo(tree)[rep50,rep50]
  tree_dist = melt_dist(tree_distM) %>%
    dplyr::rename(tree_dist=dist) 
  tree_distM_file = file.path(dir,'gene_gain_loss/rep50_dist_treeM.txt')
  print(tree_distM_file)
  write.table(tree_distM,file = tree_distM_file,quote=F,sep='\t')
  ...
  dist_brayM <- as.matrix(vegdist(t(gene_table), method="bray",full=T))
  dist_brayM = dist_brayM[rep50,rep50]
  dist_brayM_file = file.path(dir,'gene_gain_loss/rep50_dist_brayM.txt')
  print(dist_brayM_file)
  write.table(dist_brayM,file = dist_brayM_file,quote=F,sep='\t')
  mantel.test(tree_distM,dist_brayM)
  dist_bray = melt_dist(dist_brayM) %>%
...
```{rmessage=FALSE,results='hide',warning=FALSE}
