...
runs count to assess gene gains and losses 
...
```{r,warning=FALSE}
identify_pw_50strain = function(species,outfile) {
  ...
  write_tsv(pw_df,file.path(dir,outfile))
  return(pw_df)
 
}

Bxy_pw = identify_pw_50strain('Bacteroides_xylanisolvens','pw_50strain.txt')
Bov_pw = identify_pw_50strain("Bacteroides_ovatus",'pw_50strain.txt')
Bfr_pw = identify_pw_50strain("Bacteroides_fragilis",'pw_50strain.txt')
Bth_pw = identify_pw_50strain("Bacteroides_thetaiotaomicron",'pw_50strain.txt')
```
### Identify phylogenetically independent contrasts for each Bacteroides species
start with Bxy strain and determine min phylogenetic distance cutoff that gets 3 comparisons between captive ape associated strains and closest human associated strain. Determine how many sd this value is below the mean and then apply similar phylogenetic distance cutoff to other Bacteroides species. 
```{r,warning=FALSE}
get_phylo_independent_comps = function(species,cutoff,outfile) {
  ...
  write_tsv(PI_comps,file = file.path(dir,outfile))
...
Bxy_cutoff = .008
Bxy_PIC = get_phylo_independent_comps('Bacteroides_xylanisolvens',Bxy_cutoff,'PI_comps.txt')
(sd_from_mean = (Bxy_cutoff - mean(Bxy_pw$tree_dist))/sd(Bxy_pw$tree_dist))

#apply cutoff 2.1 sd below mean for other Bt species
(Bov_cutoff = mean(Bov_pw$tree_dist)+sd_from_mean*sd(Bov_pw$tree_dist))
Bov_PIC = get_phylo_independent_comps('Bacteroides_ovatus',Bov_cutoff,'PI_comps.txt')
(Bfr_cutoff = mean(Bfr_pw$tree_dist)+sd_from_mean*sd(Bfr_pw$tree_dist))
Bfr_PIC = get_phylo_independent_comps('Bacteroides_fragilis',Bfr_cutoff,'PI_comps.txt')
(Bth_cutoff = mean(Bth_pw$tree_dist)+sd_from_mean*sd(Bth_pw$tree_dist))
Bth_PIC = get_phylo_independent_comps('Bacteroides_thetaiotaomicron',Bth_cutoff,'PI_comps.txt')
```
...
pairwise_gene_gain_runner = function(iso1,iso2,species,gp,window_size) {
  ...
  countOutput_file=file.path(dir,paste0("count/countOutput_",species,"_gainpenalty",gp,".FAMILY"))
  pairwise_gene_gain(
...
pairwise_gene_gain_runner('P21.11A','GCA.003458755.1.ASM345875v1','Bacteroides_xylanisolvens',gp = 2,window_size =1)

Bxy_PIC = read_tsv('results/pangenome/Bacteroides_xylanisolvens/PI_comps.txt',col_types = cols())
...
Bov_PIC = read_tsv('results/pangenome/Bacteroides_ovatus/PI_comps.txt',col_types = cols())
mapply(pairwise_gene_gain_runner,Bov_PIC$iso1,Bov_PIC$iso2,'Bacteroides_ovatus',window_size = 5, gp = 2)
Bfr_PIC = read_tsv('results/pangenome/Bacteroides_fragilis/PI_comps.txt',col_types = cols())
mapply(pairwise_gene_gain_runner,Bfr_PIC$iso1,Bfr_PIC$iso2,'Bacteroides_fragilis',window_size = 5, gp = 2)
Bth_PIC = read_tsv('results/pangenome/Bacteroides_thetaiotaomicron/PI_comps.txt',col_types = cols())
mapply(pairwise_gene_gain_runner,Bth_PIC$iso1,Bth_PIC$iso2,'Bacteroides_thetaiotaomicron',window_size = 5, gp = 2)


get_summary_output <- function(species,suffix,folder){
  dir = paste0('results/pangenome/',species)
  pw_outdir = file.path(dir,'pairwise_gene_gain')
  
  outdir = paste0('results/pangenome/',species,'/',folder)
  dir.create(outdir)
  ...
  outfile = file.path(outdir,paste0(species,'_',suffix,'_summary.txt'))
  ...
  outfile = file.path(outdir,paste0(species,'_',suffix,'_gff.txt'))
  ...
  outfile = file.path(outdir,paste0(species,'_',suffix,'_copynum.txt'))
  ...
  outfile = file.path(outdir,paste0(species,'_',suffix,'_island.txt'))
  write_tsv(island_df,file=outfile)  
}

get_summary_output('Bacteroides_xylanisolvens','window5_gp2','pairwise_gene_gain_summary')
get_summary_output('Bacteroides_xylanisolvens','window5_gp1','pairwise_gene_gain_summary')
get_summary_output('Bacteroides_xylanisolvens','window1_gp2','pairwise_gene_gain_summary')
get_summary_output('Bacteroides_ovatus','window5_gp2','pairwise_gene_gain_summary')
get_summary_output('Bacteroides_fragilis','window5_gp2','pairwise_gene_gain_summary')
get_summary_output('Bacteroides_thetaiotaomicron','window5_gp2','pairwise_gene_gain_summary')
```