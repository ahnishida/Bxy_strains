...
---
Edits metadata, filters out low quality genomes, runs prokka on genome assemblies, sets up files to run roary pangenome, makes phylogeny, generates annotations for homologous groups
...
metadata_edit <- metadata %>% 
  filter(Contamination<5 & Completeness>95) %>%
...
dir.create('results/pangenome')
write_tsv(metadata_edit, 
          file= "results/pangenome/Bacteroides_metadata.txt")
head(metadata_edit)
```
...
Bxy_metadata <- metadata_edit %>% filter(taxonomy_Species== 'Bacteroides_xylanisolvens')
table(Bxy_metadata$host)
summary(lm_robust(Genome.size ~ Contamination + Completeness + host, data=Bxy_metadata))
summary(lm_robust(predicted.genes ~ Contamination + Completeness + host, data=Bxy_metadata))
...
```{r}
metadata = metadata %>% 
  mutate(included_analysis = ifelse(isolate %in% metadata_edit$isolate_old,1,0)) 
metadata %>% group_by(included_analysis,taxonomy_Species,dataset) %>% tally()
...
setdiff(metadata_edit$isolate_old,as.character(prokka_done))
metadata_prokka_todo <- metadata_edit %>% 
...
```{r}
dir='results/pangenome/Bacteroides_xylanisolvens'
outdir=file.path('results/analyses_input_files')
dir.create(outdir)
```
...
write_tsv(annotation, file= file.path(outdir,'Bxy_roary_nosplitparalogs_annotation.txt'))
```

#### Aggregate faas files into a single faa file
```{bash echo=F,message=FALSE,results='hide'}
ls results/pangenome/Bacteroides_xylanisolvens/faa/
cat results/pangenome/Bacteroides_xylanisolvens/faa/* > results/analyses_input_files/all_prot.faa
```

### Edit Bxy metadata, reroot Bxy strain phylogeny
...
outgroup_names <- Bxy_metadata %>% 
  filter(taxonomy_Species!='Bacteroides_xylanisolvens') %>% 
  pull(isolate)
Bxy_tree_file = file.path(dir,"phylogeny/RAxML_bipartitions.Bacteroides_xylanisolvens")
...
write.tree(Bxy_tree,file = file.path(outdir,'Bacteroides_xylanisolvens.tre'))
...
Bxy_metadata <- Bxy_metadata %>% 
  ...
  if_else(host %in% c('chimpanzee','bonobo','gorilla','orangutan'),
                                                  'ape',host))
#write out metadata
write_tsv(Bxy_metadata,file=file.path(outdir,'Bxy_metadata.txt'))
...
for (gain_penalty in c(1,1.5,2,2.5,3)) {
 runCount('Bacteroides_xylanisolvens', gain_penalty)
 runCount('Bacteroides_ovatus', gain_penalty)
 runCount('Bacteroides_fragilis', gain_penalty)
 runCount('Bacteroides_thetaiotaomicron', gain_penalty)
}


runKruskal <- function(species,gain_penalty) {
  count_outdir = file.path('results/pangenome',species,'count')
  count_outfile = file.path(count_outdir,paste0('countOutput_',species,'_gainpenalty',gain_penalty))
  present <- read.table(paste0(count_outfile,".PRESENT"),sep='\t',header=TRUE)
  res <- present %>% 
    mutate(istip = if_else(str_detect(node,'Node'),'node','tip')) %>% 
...
Bxy = rbind(
  runKruskal('Bacteroides_xylanisolvens',1),
  runKruskal('Bacteroides_xylanisolvens',1.5),
  runKruskal('Bacteroides_xylanisolvens',2),
  runKruskal('Bacteroides_xylanisolvens',2.5),
  runKruskal('Bacteroides_xylanisolvens',3))

Bov = rbind(
  runKruskal('Bacteroides_ovatus',1),
  runKruskal('Bacteroides_ovatus',1.5),
  runKruskal('Bacteroides_ovatus',2),
  runKruskal('Bacteroides_ovatus',2.5),
  runKruskal('Bacteroides_ovatus',3))

Bfr = rbind(
  runKruskal('Bacteroides_fragilis',1),
  runKruskal('Bacteroides_fragilis',1.5),
  runKruskal('Bacteroides_fragilis',2),
  runKruskal('Bacteroides_fragilis',2.5),
  runKruskal('Bacteroides_fragilis',3))

Bth = rbind(
  runKruskal('Bacteroides_thetaiotaomicron',1),
  runKruskal('Bacteroides_thetaiotaomicron',1.5),
  runKruskal('Bacteroides_thetaiotaomicron',2),
  runKruskal('Bacteroides_thetaiotaomicron',2.5),
  runKruskal('Bacteroides_thetaiotaomicron',3))
